...
app.on("ui_update", function () {
  ...
  for (i = 1; i <= 4; i++) {
    ...
    if (default_setting.display_as_money) {
      //Calculate balance to money if display_as_money is true
      let display_balance =
        Math.round(allplayer[i].balance * default_setting.money * 100) / 100;
      let unrealized_balance =
        Math.round(allplayer[i].unrealized * default_setting.money * 100) / 100;
      allplayer[i].unrealized == 0
        ? $(`.p${i}_balance`).text(`$${display_balance}`)
        : allplayer[i].unrealized > 0
          ? $(`.p${i}_balance`).text(
              `$${display_balance} (+$${unrealized_balance})`,
            )
          : $(`.p${i}_balance`).text(
              `$${display_balance} ($${unrealized_balance})`,
            );
      ...
      allplayer[i].unrealized == 0
        ? $(`.p${i}_balance`).text(allplayer[i].balance)
        : allplayer[i].unrealized > 0
          ? $(`.p${i}_balance`).text(
              `${allplayer[i].balance} (+${allplayer[i].unrealized})`,
            )
          : $(`.p${i}_balance`).text(
              `${allplayer[i].balance} (${allplayer[i].unrealized})`,
            );
    ...
    for (x = 1; x <= 4; x++) {
      ...
      if (
        parseInt(allplayer[i]["sf" + x]) > 0 &&
        parseInt(allplayer[i]["sf" + x]) % default_setting.break == 0
      ) {
        //Copy streak warning node if streak % break = 0
        $("#p" + i + "-action-break").removeClass("none");
        let streak_warning = $(".exclamation")[0].cloneNode(true);
        switch (allplayer[i].position) {
          case "E":
            $("#east").append(streak_warning);
            break;
          case "S":
            $("#south").append(streak_warning);
            break;
          case "W":
            $("#west").append(streak_warning);
            break;
          case "N":
            $("#north").append(streak_warning);
            break;
          default:
            console.error(
              "Error while applying streak warning sign: Unknown case",
            );
        }
      }
      //Add dot animations
      if (parseInt(allplayer[i]["sf" + x]) > 0) {
        for (streak = 1; streak <= allplayer[i]["sf" + x]; streak++) {
          ...
          animation_dots =
            animation_dots[animation_dots.length - 1].cloneNode();
    ...
    if (gamestat.banker == allplayer[i].position) {
      switch (allplayer[i].position) {
        case "E":
          $("#east").addClass("banker_block");
          gamestat.streak > 0
            ? $("#east").html(`<h1>${gamestat.streak}</h1>`)
            : null;
          break;
        case "S":
          $("#south").addClass("banker_block");
          gamestat.streak > 0
            ? $("#south").html(`<h1>${gamestat.streak}</h1>`)
            : null;
          break;
        case "W":
          $("#west").addClass("banker_block");
          gamestat.streak > 0
            ? $("#west").html(`<h1>${gamestat.streak}</h1>`)
            : null;
          break;
        case "N":
          $("#north").addClass("banker_block");
          gamestat.streak > 0
            ? $("#north").html(`<h1>${gamestat.streak}</h1>`)
            : null;
          break;
        default:
          console.error("Error while adding banker animation: Unknown case");
      }
    }
    ...
    parseInt(gamestat.round) > 1
      ? $(`.p${i}-win-ratio`).text(
          round_to_1_dec(
            (parseInt(allplayer[i].win) / parseInt(gamestat.round - 1)) * 100,
          ) + "%",
        )
      : null;
    parseInt(gamestat.round) > 1
      ? $(`.p${i}-lose-ratio`).text(
          round_to_1_dec(
            (parseInt(allplayer[i].lose) / parseInt(gamestat.round - 1)) * 100,
          ) + "%",
        )
      : null;
    parseInt(gamestat.tsumo) > 0
      ? $(`.p${i}-tsumo-ratio`).text(
          round_to_1_dec(
            (parseInt(allplayer[i].tsumo) / parseInt(gamestat.tsumo)) * 100,
          ) + "%",
        )
      : null;
    parseInt(gamestat.deal) > 0
      ? $(`.p${i}-deal-lose-ratio`).text(
          round_to_1_dec(
            (parseInt(allplayer[i].deal_lose) / parseInt(gamestat.deal)) * 100,
          ) + "%",
        )
      : null;
    parseInt(gamestat.instant_get) > 0
      ? $(`.p${i}-instant-get-ratio`).text(
          round_to_1_dec(
            (parseInt(allplayer[i].instant_get) /
              parseInt(gamestat.instant_get)) *
              100,
          ) + "%",
        )
      : null;
    parseInt(gamestat.instant_pay) > 0
      ? $(`.p${i}-instant-pay-ratio`).text(
          round_to_1_dec(
            (parseInt(allplayer[i].instant_pay) /
              parseInt(gamestat.instant_pay)) *
              100,
          ) + "%",
        )
      : null;
  ...
  switch (Math.floor((gamestat.round_prevailing - (round - 1) * 16 - 1) / 4)) {
    case 0:
      round_prevailing_text += "東圈";
      break;
    case 1:
      round_prevailing_text += "南圈";
      break;
    case 2:
      round_prevailing_text += "西圈";
      break;
    case 3:
      round_prevailing_text += "北圈";
      break;
    default:
      console.error("Error while applying round text: Unknown case");
  }
  switch (gamestat.round_prevailing % 4) {
    case 0:
      round_prevailing_text += "北";
      break;
    case 1:
      round_prevailing_text += "東";
      break;
    case 2:
      round_prevailing_text += "南";
      break;
    case 3:
      round_prevailing_text += "西";
      break;
    default:
      console.error("Error while applying round text: Unknown case");
  }
  ...
  undo_count >= fulldata_JSON.length - 1
  ...
  if (gamestat.round > 1) {
    $(".draw_ratio").text(
      round_to_2_dec((gamestat.tie / (gamestat.round - 1)) * 100) + "%",
    );
    $(".deal_ratio").text(
      round_to_2_dec((gamestat.deal / (gamestat.round - 1)) * 100) + "%",
    );
    $(".tsumo_ratio").text(
      round_to_2_dec((gamestat.tsumo / (gamestat.round - 1)) * 100) + "%",
    );
    $(".double_ratio").text(
      round_to_2_dec((gamestat.double_winner / (gamestat.round - 1)) * 100) +
        "%",
    );
    $(".triple_ratio").text(
      round_to_2_dec((gamestat.triple_winner / (gamestat.round - 1)) * 100) +
        "%",
    );
  }
...
app.on("pageInit", function () {
  ...
  $(".quick-actions .actions-button").on("click", function () {
    //Event listeners for closing actions after click
    app.actions.close();
  });
});
...
window.onload = () => {
  if (load()) {
    let now = new Date();
    let days_since_last_save =
      Math.abs(now - Date.parse(gamestat.last_save)) / (1000 * 3600 * 24);
    if (days_since_last_save > 2) {
      app.dialog.confirm(
        "上一次存取記錄已是" +
          Math.floor(days_since_last_save) +
          "日前<br>是否清除資料並開始開局？",
        "清除資料",
        function () {
...
function fill_names() {
  ...
  for (i = 1; i <= 4; i++) {
    //Set position in css for animations
...
function open_action_sheet(player_index) {
  switch (player_index) {
    case 1:
      app.actions.open(".p1-actions");
      break;
    case 2:
      app.actions.open(".p2-actions");
      break;
    case 3:
      app.actions.open(".p3-actions");
      break;
    case 4:
      app.actions.open(".p4-actions");
      break;
    default:
      console.error("Error while opening player action sheet: Unknown case");
  }
}
...
function create_break_dialog(player_index) {
  ...
  for (i = 1; i <= 4; i++) {
    if (
      allplayer[player_index]["sf" + i] > 0 &&
      allplayer[player_index]["sf" + i] % default_setting.break == 0
    ) {
  ...
  app.dialog.confirm(
    ...
    "中斷拉踢",
    function () {
      end_streak(player_index, receiver_index);
    },
    function () {
      app.dialog.close();
    },
...
function roll_dice() {
  ...
  let dice_array = [];
  for (i = 0; i <= 14; i++) {
    //Randomize 15 dices and put it into an array
    dice_array.push(Math.ceil(Math.random() * 6));
  }
  let dice_sum =
    dice_array[dice_array.length - 2] +
    dice_array[dice_array.length - 3] +
    dice_array[dice_array.length - 4]; //Only pick last 2nd, 3rd, 4th for the sum
  // Generate html icons for displaying dice animation
  let dice_html = "";
  for (dice of dice_array) {
    dice_html +=
      dice == 1
        ? `<i class='material-icons dice'>looks_one</i>`
        : dice == 2
          ? `<i class='material-icons dice'>looks_two</i>`
...
// ------------------------------------------ //
// Starting Modal Functions
function set_money_multiplier(value) {
  //Check money multiplier input before setting it as value
  let check_value = parseFloat(value);
  if (isNaN(check_value) || check_value <= 0 || check_value == "") {
    //Throw toast error if input is invalid
...
function set_break_streak(value) {
  //Check break streak input before setting it as value
  let check_value = parseInt(value);
  if (check_value == -1) {
    $('#start-form input[name="break_streak"]').val(check_value);
  } else if (isNaN(check_value) || check_value <= 0 || check_value == "") {
    //Throw toast error if input is invalid
...
function submit_start_form() {
  //Function for handling start form submit
  ...
  start_obj.break_streak =
    start_form.elements["break_streak"].value == -1
      ? Infinity
      : parseInt(start_form.elements["break_streak"].value);
...
function set_instantGet_position(player_selected) {
  ...
  for (i = 1; i <= 4; i++) {
    if (i == player_selected) {
      //Append selected player into specific position
...
function submit_instantGet_form() {
  ...
  for (i = 1; i <= 4; i++) {
    //Search for index of selected player
    if ($(".instantGet_selected div").hasClass(`p${i}_card`)) {
      instantGet_object.selected = i;
      break;
    }
  }
  instantGet_object.value = $(
    '#instantGet-form input[name="instant-get"]:checked',
  ).val();
...
function submit_instantPay_form() {
  ...
  instantPay_object.value = $(
    '#instantPay-form input[name="instant-pay"]:checked',
  ).val();
...
function set_deal_position(player_selected) {
  ...
  for (i = 1; i <= 4; i++) {
    if (i == player_selected) {
      ...
      //Append other players into corresponding positions
...
function submit_deal_form() {
  ...
  for (i = 1; i <= 4; i++) {
    if (i == deal_object.selected) {
      // When the player is selected player
      if (
        $(`#p${i}_deal_input`).val() == "" ||
        $(`#p${i}_deal_input`).val() == 0
      ) {
        //Throw error if no input or input = 0
...
// Function for auto filling all the tsumo input with only 1 given value
function tsumo_autofill() {
  const positions = ["left", "center", "right"];
  let val_without_banker;
  for (position of positions) {
    const input = Number($(`.tsumo_${position} input`).val());
    const is_banker = $(`.tsumo_${position} .card`).hasClass("banker");
    if (input !== 0) {
      if (is_banker) {
        val_without_banker = input - (2 * gamestat.streak + 1);
      } else {
        val_without_banker = input;
      }
      break;
    }
  }
  if (!val_without_banker) {
    catch_error("必須先填寫其中一家的番數");
    return;
  }
  for (position of positions) {
    if ($(`.tsumo_${position} .card`).hasClass("banker")) {
      $(`.tsumo_${position} input`).val(
        val_without_banker + 2 * gamestat.streak + 1,
      );
    } else {
      $(`.tsumo_${position} input`).val(val_without_banker);
    }
  }
  tsumo_input_actions();
}
...
function submit_tsumo_form() {
  ...
  for (i = 1; i <= 4; i++) {
    if (i != tsumo_object.selected) {
      // When the player is selected player
      if (
        $(`#p${i}_tsumo_input`).val() == "" ||
        $(`#p${i}_tsumo_input`).val() == 0
      ) {
        //Throw error if no input or input = 0
...
function fill_summary() {
  ...
  for (i = 0; i <= 3; i++) {
    //Add unrealized into balance for all players
    summary_allplayer[i].balance += summary_allplayer[i].unrealized;
    summary_allplayer[i].money = round_to_2_dec(
      summary_allplayer[i].balance * default_setting.money,
    );
  }
  ...
  summary.max_yaku.index =
    mapped[
      summary_allplayer[
        summary.max_yaku.map.indexOf(summary.max_yaku.max)
      ].position
    ];
  // Tsumo
  ...
  summary.win.index =
    mapped[
      summary_allplayer[summary.win.map.indexOf(summary.win.max)].position
    ];
  // PasserBy
  summary.passerBy.map = summary_allplayer.map(
    (x) => parseInt(x.deal_lose) + parseInt(x.win),
  );
  summary.passerBy.max = Math.min(...summary.passerBy.map);
  summary.passerBy.index =
    mapped[
      summary_allplayer[
        summary.passerBy.map.indexOf(summary.passerBy.max)
      ].position
    ];
  for (i = 0; i <= 3; i++) {
...
function display_custom(seat_boolean) {
  //Show seats when custom is selected
  if (seat_boolean) {
    ...
    $(".custom-seat-east, .custom-seat-south, .custom-seat-north").addClass(
      "none",
    );
...
function east_trigger() {
  ...
  let append_html = "";
  for (i = 1; i <= 4; i++) {
    i != $('input[name="seat-east"]:checked').val()
      ? (append_html += `
        <li>
            <label class="item-radio item-radio-icon-start item-content">
...
function south_trigger() {
  ...
  for (i = 1; i <= 4; i++) {
    i != $('input[name="seat-east"]:checked').val() &&
    i != $('input[name="seat-south"]:checked').val()
      ? (append_html += `
        <li>
            <label class="item-radio item-radio-icon-start item-content">
...
function west_trigger() {
  ...
  for (i = 1; i <= 4; i++) {
    i != $('input[name="seat-east"]:checked').val() &&
    i != $('input[name="seat-south"]:checked').val() &&
    i != $('input[name="seat-west"]:checked').val()
      ? (append_html += `
        <li>
            <label class="item-radio item-radio-icon-start item-content">
...
function validate_import() {
  try {
    ...
    for (i = 1; i <= 4; i++) {
      import_summary +=
        temp_data.allplayer[i].name +
        " ： " +
        temp_data.allplayer[i].balance +
        "(" +
        temp_data.allplayer[i].unrealized +
        ")<br>";
    }
    import_summary +=
      "<br>匯入資料後，現時的遊遊資料將會被覆蓋<br>確定要匯入嗎？";
    app.dialog.confirm(
      import_summary,
      "匯入資料",
      function () {
...
$(document).on("popup:open", ".log-popup", function () {
  ...
  for (i = 0; i < game_log.length; i++) {
    game_log[i].removed
      ? (append_log +=
          '<tr><td class="log text-align-center removed">' +
          game_log[i].timestamp +
          '</td><td class="log removed">' +
          game_log[i].message +
          "</td></tr>")
      : (append_log +=
          '<tr><td class="log text-align-center">' +
          game_log[i].timestamp +
          '</td><td class="log">' +
          game_log[i].message +
          "</td></tr>");
...
app.on("adjust_change", function () {
  ...
  if (manual_adjust_array.length == 0) {
    adjust_html =
      '<li><div class="item-content"><div class="item-inner"><div class="item-title">尚未有行動</div></div></div></li>';
  } else {
    for (i = 0; i < manual_adjust_array.length; i++) {
      let player_name = allplayer[manual_adjust_array[i]["player_index"]].name;
      let action_text =
        manual_adjust_array[i]["action"] == "add"
          ? " 增加"
          : manual_adjust_array[i]["action"] == "minus"
            ? " 減少"
            : manual_adjust_array[i]["action"] == "equal"
              ? " 的番數設定為"
              : null;
      let adjust_message =
        player_name + action_text + manual_adjust_array[i].value + "番";
      adjust_html +=
        '<li><div class="item-content"><div class="item-media"><button class="button" href="#"  onclick="remove_adjust(' +
        i +
        ')"><i class="f7-icons if-not-md">xmark</i><i class="material-icons md-only">close</i></button></div><div class="item-inner"><div class="item-title">' +
        adjust_message +
        "</div></div></div></li>";
    }
  }
