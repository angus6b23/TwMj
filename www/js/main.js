...
function tsumo(player_index, game_arr) {
	...
	for (i = 1; i <= 4; i++) { //Find for max yaku and generate log
		if (game_arr[i] < 0) {
			...
			(Math.abs(game_arr[i]) > tsumo_max_yaku) ? tsumo_max_yaku = Math.abs(game_arr[i]) : null; //Record largest yaku for calculating average yaku for game stats
		}
	}
	...
	if (default_setting.tsumo_method === 'even') {
		...
		let half_arr = game_arr.map(function (yaku) {
			if (yaku < 0) {
				let yaku_half = Math.floor(yaku / 2);
				total += Math.abs(yaku_half);
				return yaku_half;
			}
		});
...
async function capture() {
	...
	try {
		setTimeout(async () => {
			...
			let image = await capturePromise(element);
			await image.toBlob(function (blob) {
